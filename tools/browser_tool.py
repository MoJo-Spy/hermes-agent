...
def _managed_chrome_executable() -> Optional[Path]:
    managed = get_hermes_home() / "playwright"
    if not managed.is_dir():
        return None
    name = "chrome.exe" if sys.platform == "win32" else "chrome"
    return next((path for path in managed.rglob(name) if path.is_file()), None)
...
def _build_browser_env() -> dict:
    ...
    configured_playwright = os.environ.get("PLAYWRIGHT_BROWSERS_PATH", "").strip()
    managed_playwright = get_hermes_home() / "playwright"
    if configured_playwright and configured_playwright != "0":
        env["PLAYWRIGHT_BROWSERS_PATH"] = configured_playwright
    elif managed_playwright.is_dir():
        env["PLAYWRIGHT_BROWSERS_PATH"] = str(managed_playwright)
    configured_chrome = os.environ.get("AGENT_BROWSER_EXECUTABLE_PATH", "").strip()
    managed_chrome = _managed_chrome_executable()
    if configured_chrome:
        env["AGENT_BROWSER_EXECUTABLE_PATH"] = configured_chrome
    elif managed_chrome:
        env["AGENT_BROWSER_EXECUTABLE_PATH"] = str(managed_chrome)
    return env
...
def _chromium_search_roots() -> List[str]:
    ...
    2. ``HERMES_HOME/playwright`` — Hermes' managed offline browser cache.
    ...
    roots.append(str(get_hermes_home() / "playwright"))
